...
all.pages <- pdf_text("original-data/november/2004-11-02_FallElection_USCongress_WardbyWard.pdf")
...
all.df.final <- all.df %>%
  ...
  mutate(
    ...
    candidate = case_when(
      ...
      district == 6 & party == "Wisconsin Green" ~ "Carol Ann Rittenhouse",
      district == 7 & party == "Democratic" ~ "David R Obey",
      district == 7 & party == "Wisconsin Green" ~ "Mike Miles",
...
all.df.final.renamed <- all.df.final |>
  rename(municipality = municipality_name, reporting_unit = reporting_unit_name,
         ctv = municipality_type)

write_csv(all.df.final.renamed, "processed-data/november/annual/2004-congress-only.csv")
