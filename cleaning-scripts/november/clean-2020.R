...
pres.2020.clean <- pres.2020.orig |>
  ...
  separate(reporting_unit, into = c("municipality_name", "reporting_unit_name"), sep = " (?=Ward|WARD)") |>
...
read_wsa_dist <- function(sheet){
  thispath <- "original-data/2020_Ward%20by%20Ward%20Report%20-%20Representative%20to%20the%20Assembly.xlsx"
...
read_wss_dist <- function(sheet){
  thispath <- "original-data/2020_Ward%20by%20Ward%20Report%20-%20State%20Senate.xlsx"
  dist <- readxl::read_excel(thispath, sheet = sheet, col_names = F)
...
all.wss.dist.orig <- map_df(2:17, read_wss_dist)
