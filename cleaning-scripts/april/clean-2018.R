...
# the april 2018 nonpartisan (spring) general election
orig.path <- "original-data/april/Ward_20Report-4.3.18_20Spring_20Election-Supreme_20Court_2018.xlsx"

# the workbook holds only the JUSTICE OF THE SUPREME COURT contest (Screnock
# vs. Dallet). layout is the same as the february nonpartisan workbooks: the
...
sheets.2018 <- lapply(seq_along(readxl::excel_sheets(orig.path)), read_sheet_long)
...
long.2018 <- bind_rows(map(sheets.2018, "votes")) |>
  ...
  ungroup() |>
  # quirk: the workbook lists the town of MENOMINEE's reporting units twice —
  # once with the actual votes and once as all-zero rows (Menominee county
  # overlaps the Menominee Indian Reservation, and WEC reports parallel ward
  # lines). keep the first occurrence of each reporting unit, which is the
  # one with the votes; the all-zero duplicates add nothing, so the office-
  # totals check below still passes either way
...
results.2018 |> count(office, candidate, wt = votes)
...
write_csv(results.2018, "processed-data/april/annual/2018.csv")
