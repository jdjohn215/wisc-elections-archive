...
# the april 2000 nonpartisan (spring) general election
orig.path <- "original-data/april/2000-04-04_Spring_SupremeCourt_WardbyWard_2000.pdf"
...
# is computed as the candidate vote sum per reporting unit. 2000 was not a
...
read_pdf_page <- function(page) {
  ...
  d3 |>
    ...
    separate(x1, into = c("rep_unit", "diane s sykes", "louis b butler", "Scattering"),
             sep = " {2,}") |>
...
stopifnot(identical(names(results.2000), names(template)))

# no failed numeric conversions
stopifnot(!any(is.na(results.2000$votes)), !any(is.na(results.2000$total_votes)))
...
results.2000 |> summarise(votes = sum(votes), .by = candidate)
results.2000 |> summarise(total = sum(votes))
readxl::read_excel("processed-data/nonpartisan/race-totals.xlsx") |>
  filter(year == 2000)
stopifnot(abs(sum(results.2000$votes) -
  (readxl::read_excel("processed-data/nonpartisan/race-totals.xlsx") |>
     filter(year == 2000) |> pull(total))) < 1000)

dir.create("processed-data/april/annual", recursive = TRUE, showWarnings = FALSE)
write_csv(results.2000, "processed-data/april/annual/2000.csv")
