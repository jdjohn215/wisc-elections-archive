...
# the april 2009 nonpartisan (spring) general election
orig.path <- "original-data/april/2009-04-07_SpringElection_SupCt_WardbyWard_2009.xls"
...
#
# this file holds the JUSTICE OF THE SUPREME COURT contest only (Koschnick
# vs. Abrahamson). 2009 was also a superintendent race year (Evers vs.
# Fernandez), but this file does not contain it; a separate source is
...
# candidate display names come from header row 1 ("Randy R. Koschnick",
# "Shirley S. Abrahamson", "Scattering"); row 2 holds the party label, which
# is ignored — every row gets party = "NONPARTISAN"
...
stopifnot(identical(names(results.2009), names(template)))

# no failed numeric conversions
stopifnot(!any(is.na(results.2009$votes)), !any(is.na(results.2009$total_votes)))
...
totals.cmp <- full_join(certified,
                       results.2009 |> summarise(votes = sum(votes), .by = candidate),
                       by = "candidate")
...
write_csv(results.2009, "processed-data/april/annual/2009.csv")
