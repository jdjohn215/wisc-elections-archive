...
# the april 2008 nonpartisan (spring) general election
orig.path <- "original-data/april/2008-04-01_SpringElection_SupCt_WardbyWard_2008.xls"
...
#
# this file holds the JUSTICE OF THE SUPREME COURT contest only (Gableman
# vs. Butler). the workbook's second and third sheets are empty stubs —
# read sheet 1 explicitly.
#
...
# candidate display names come from header row 1 ("Mike Gableman",
# "Louis Butler", "Scattering"); row 2 holds the party label, which is
...
stopifnot(identical(names(results.2008), names(template)))

# no failed numeric conversions
stopifnot(!any(is.na(results.2008$votes)), !any(is.na(results.2008$total_votes)))
...
totals.cmp <- full_join(certified,
                       results.2008 |> summarise(votes = sum(votes), .by = candidate),
                       by = "candidate")
...
write_csv(results.2008, "processed-data/april/annual/2008.csv")
