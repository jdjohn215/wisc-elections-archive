...
# the april 2006 nonpartisan (spring) general election
orig.path <- "original-data/april/2006-04-04_SpringElection_SupCt_WardbyWard_2006.xls"
...
#
# this file holds the JUSTICE OF THE SUPREME COURT contest only (Crooks
# unopposed).
#
...
# candidate display names come from header row 1 ("Patrick Crooks",
...
stopifnot(identical(names(results.2006), names(template)))

# no failed numeric conversions
stopifnot(!any(is.na(results.2006$votes)), !any(is.na(results.2006$total_votes)))
...
totals.cmp <- full_join(certified,
                       results.2006 |> summarise(votes = sum(votes), .by = candidate),
                       by = "candidate")
...
write_csv(results.2006, "processed-data/april/annual/2006.csv")
