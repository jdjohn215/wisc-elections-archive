...
# the april 2019 nonpartisan (spring) general election
orig.path <- "original-data/april/Spring_20Election_204.2.19-WxW_20Report-Supreme_20Court_2019.xlsx"

# the workbook holds only the JUSTICE OF THE SUPREME COURT contest (Hagedorn
# vs. Neubauer). layout is the same as the february nonpartisan workbooks: the
# contest title sits a variable number of rows above the "Total Votes Cast"
# anchor in column 3 (taken as the last non-empty cell in column 1 above the
# anchor), party codes ("NP") sit on the anchor row and candidate names
# directly below it, and trailing "County Totals:"/"Office Totals:" rows
# bracket the ward table. party codes are ignored — every row gets
# party = "NONPARTISAN". source candidate names contain double spaces ("Lisa
# Neubauer"); kept as-is per the 2018 precedent — only str_to_upper() is
# applied.
...
sheets.2019 <- lapply(seq_along(readxl::excel_sheets(orig.path)), read_sheet_long)

long.2019 <- bind_rows(map(sheets.2019, "votes")) |>
...
# Totals:" row values
office.totals <- bind_rows(map(sheets.2019, "totals"))
vote.sums <- long.2019 |>
...
stopifnot(identical(names(results.2019), names(template)))

# no failed numeric conversions
stopifnot(!any(is.na(results.2019$votes)), !any(is.na(results.2019$total_votes)))

# make.unique() only intervenes if a contest lists the same candidate name
# twice, which shouldn't happen
stopifnot(!any(str_detect(results.2019$candidate, "__dup")))
...
results.2019 |> count(office, candidate, wt = votes)
...
write_csv(results.2019, "processed-data/april/annual/2019.csv")
