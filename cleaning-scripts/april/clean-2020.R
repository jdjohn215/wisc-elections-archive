...
# the april 2020 nonpartisan (spring) general election
orig.path <- "original-data/april/Ward_20by_20Ward_20Report_Supreme_20Court_2020.xlsx"

# the workbook holds only the JUSTICE OF THE SUPREME COURT contest (Karofsky
# vs. Kelly, held during the april 2020 presidential preference primary amid
# the covid-19 pandemic). the presidential preference vote itself is a
# separate workbook and is out of scope. layout is the same as the february
# nonpartisan workbooks: the contest title sits a variable number of rows
# above the "Total Votes Cast" anchor in column 3 (taken as the last non-
# empty cell in column 1 above the anchor), party codes ("NP") sit on the
# anchor row and candidate names directly below it, and trailing "County
# Totals:"/"Office Totals:" rows bracket the ward table. party codes are
# ignored — every row gets party = "NONPARTISAN". source candidate names
# contain double spaces ("Daniel  Kelly"); kept as-is per the 2018 precedent —
...
sheets.2020 <- lapply(seq_along(readxl::excel_sheets(orig.path)), read_sheet_long)
...
results.2020 |> count(office, candidate, wt = votes)
