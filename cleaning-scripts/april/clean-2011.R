...
# the april 2011 nonpartisan (spring) general election
orig.path <- "original-data/april/2011-04-05_Ward_results_supreme_recount_2011.xls"

# the workbook holds only the JUSTICE OF THE SUPREME COURT contest (Prosser
# vs. Kloppenburg), reported from the post-election recount. layout is the
# same as the february nonpartisan workbooks: the contest title sits a
# variable number of rows above the "Total Votes Cast" anchor in column 3
# (taken as the last non-empty cell in column 1 above the anchor), party
# codes sit on the anchor row and candidate names directly below it, and
# trailing "County Totals:"/"Office Totals:" rows bracket the ward table.
# party codes are ignored — every row gets party = "NONPARTISAN".
...
sheets.2011 <- lapply(seq_along(readxl::excel_sheets(orig.path)), read_sheet_long)

long.2011 <- bind_rows(map(sheets.2011, "votes")) |>
...
# Totals:" row values
office.totals <- bind_rows(map(sheets.2011, "totals"))
vote.sums <- long.2011 |>
...
stopifnot(identical(names(results.2011), names(template)))

# no failed numeric conversions
stopifnot(!any(is.na(results.2011$votes)), !any(is.na(results.2011$total_votes)))

# make.unique() only intervenes if a contest lists the same candidate name
# twice, which shouldn't happen
stopifnot(!any(str_detect(results.2011$candidate, "__dup")))
...
results.2011 |> count(office, candidate, wt = votes)
...
write_csv(results.2011, "processed-data/april/annual/2011.csv")
