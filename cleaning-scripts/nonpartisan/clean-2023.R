...
orig <- readxl::read_excel("original-data/nonpartisan/Ward by Ward Report statewide contests_2023.xlsx",
                           sheet = 3, col_names = F)

# read the original sheet in its entirety
s1 <- readxl::read_excel("original-data/nonpartisan/Ward by Ward Report statewide contests_2023.xlsx",
...
s2 <- s1 |>
  ...
  mutate(party = case_when(
    candidate == "Janet C. Protasiewicz" ~ "liberal",
    candidate == "Daniel Kelly" ~ "conservative",
    TRUE ~ "scattering"
  )) |>
...
write_csv(s2, "processed-data/nonpartisan/annual/2023.csv")
