...
s1 <- readxl::read_excel("original-data/nonpartisan/Supreme_20Court_20Ward_20by_20Ward_20AMENDED_20Report-Brown_20Co_2016.xlsx",
...
s2 <- s1 |>
  ...
  mutate(party = case_when(
    candidate == "Rebecca G. Bradley" ~ "conservative",
    candidate == "JoAnne F. Kloppenburg" ~ "liberal",
    TRUE ~ "Scattering"
  )) |>
  mutate(across(where(is.character), str_to_upper)) |>
  separate(reporting_unit, into = c("municipality", "reporting_unit"), sep = " (?=WARD)", extra = "merge") |>
...
write_csv(s2, "processed-data/nonpartisan/annual/2016.csv")
