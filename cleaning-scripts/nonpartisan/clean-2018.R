...
s1 <- readxl::read_excel("original-data/nonpartisan/Ward_20Report-4.3.18_20Spring_20Election-Supreme_20Court_2018.xlsx",
...
s2 <- s1 |>
  ...
  mutate(party = case_when(
    candidate == "Rebecca Dallet" ~ "liberal",
    candidate == "Michael Screnock" ~ "conservative",
    TRUE ~ "Scattering"
  )) |>
  ...
  mutate(reporting_unit = if_else(is.na(reporting_unit), "WARD 1", reporting_unit),
         ...
         year = 2018) |>
  # munge T Menominee duplicates
  group_by(county, municipality, ctv, reporting_unit, office, party) |>
  filter(total_votes == max(total_votes)) |>
  ungroup()

write_csv(s2, "processed-data/nonpartisan/annual/2018.csv")
