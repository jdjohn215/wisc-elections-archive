...
orig <- readxl::read_excel("original-data/nonpartisan/Ward by Ward Report_Spring Election 2026_All State Contests.xlsx",
                           sheet = 2, col_names = F)

# read the original sheet in its entirety
s1 <- readxl::read_excel("original-data/nonpartisan/Ward by Ward Report_Spring Election 2026_All State Contests.xlsx",
...
s2 <- s1 |>
  ...
  mutate(party = case_when(
    candidate == "Chris Taylor" ~ "liberal",
    candidate == "Maria S. Lazar" ~ "conservative",
    TRUE ~ "scattering"
  )) |>
...
write_csv(s2, "processed-data/nonpartisan/annual/2026.csv")
