...
all.pages <- pdf_text("original-data/nonpartisan/2000-04-04_Spring_SupremeCourt_WardbyWard_2000.pdf")
...
all.df.final <- all.df %>%
  ...
  mutate(
    party = case_when(
      candidate == "diane s sykes" ~ "Conservative",
      candidate == "louis b butler" ~ "Liberal",
...
write_csv(all.df.final, "processed-data/nonpartisan/annual/2000.csv")
