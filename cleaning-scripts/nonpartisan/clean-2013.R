...
s1 <- readxl::read_excel("original-data/nonpartisan/Ward_20Results_4.2.13_20Spring_20Election_Supreme_20Court_20Justice_2013.xls",
...
s2 <- s1 |>
  ...
  mutate(party = case_when(
    ...
    candidate == "ED FALLONE" ~ "liberal",
    TRUE ~ "Scattering"
  )) |>
  mutate(across(where(is.character), str_to_upper)) |>
  separate(reporting_unit, into = c("municipality", "reporting_unit"), sep = " (?=WARD)| (?=WD)") |>
...
write_csv(s2, "processed-data/nonpartisan/annual/2013.csv")
