...
# the february 2018 nonpartisan (spring) primary
orig.path <- "original-data/february/2018_Ward_20by_20Ward.all_20offices.spring_20primary.2.20.18.xlsx"

# same header-block layout as the 2025 workbook (see 2025.R), with two
# differences:
# - the contest title's offset from the "Total Votes Cast" anchor varies
#   between sheets within this workbook (2 rows above on the supreme court
#   sheet, 3 on the circuit court sheets), so the title is taken as the last
#   non-empty cell in column 1 above the anchor instead of a fixed offset
# - the workbook is "all offices", so it also holds two county circuit court
#   primaries (Columbia County branch 3, Manitowoc County branch 2), which
#   the office filter drops
#
# only STATE SUPERINTENDENT OF PUBLIC INSTRUCTION and supreme court contests
# are in scope; this workbook's only in-scope contest is JUSTICE OF THE
# SUPREME COURT (Rebecca Dallet, Michael Screnock, Tim Burns)
...
results.2018 <- long.2018 |>
  ...
  # (this drops the two circuit court contests)
...
# be matched against a 2018 file
...
# spot check the supreme court primary
results.2018 |>
  group_by(candidate) |>
  summarise(votes = sum(votes), .groups = "drop")

dir.create("processed-data/february/annual", recursive = TRUE, showWarnings = FALSE)
write_csv(results.2018, "processed-data/february/annual/2018.csv")
