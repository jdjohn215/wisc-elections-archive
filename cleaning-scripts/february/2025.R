...
# the february 2025 nonpartisan (spring) primary
orig.path <- "original-data/february/2025_Ward by Ward Report.xlsx"

# this workbook differs from the august "all contests" files in that it is a
# nonpartisan primary with one sheet per contest and no document map:
# - the contest title (e.g. "STATE SUPERINTENDENT OF PUBLIC INSTRUCTION") sits
#   a variable number of rows above the "Total Votes Cast" anchor cell in
#   column 3 (the offset varies between sheets in other february workbooks),
#   so it is taken as the last non-empty cell in column 1 above the anchor
...
#   conversion failures
# - source candidate names contain double spaces ("Jeff  Wright"); kept as-is
...
# are in scope; the 2025 workbook holds just the superintendent, but the
# supreme court pattern matches how WEC titled the 2023 february contest
# ("JUSTICE OF THE SUPREME COURT") so future years keep working
...
write_csv(results.2025, "processed-data/february/annual/2025.csv")
