...
# the february 2023 nonpartisan (spring) primary
orig.path <- "original-data/february/2023_Ward by Ward Report_Spring Primary_2_21_23.xlsx"

# same header-block layout as the 2025 workbook (see 2025.R), with two
# differences:
# - a document map sheet plus eight contest sheets, the last of which is an
#   empty one-column stub (7 contests listed, 8 data sheets) — the ncol < 3
#   guard skips the map and the stub
# - the workbook also holds four partisan special-election primaries for
#   STATE SENATOR DISTRICT 8 and two county circuit court primaries, which
#   the office filter drops; note this workbook has NO state superintendent
#   contest because no superintendent primary was held in february 2023 —
#   the April 2023 race was a two-candidate general (Underly vs. Kerr) that
#   went straight to the spring election
#
...
results.2023 <- long.2023 |>
  ...
  # (this drops the special-election senate and circuit court contests)
...
write_csv(results.2023, "processed-data/february/annual/2023.csv")
